...
from scripts.model_comparison import lr_rmse, lstm_rmse  # Import RMSE values
import joblib
...
def predict_stock_price():
    try:
        ...
        # Reshape and scale the input for the LSTM model
        ...
        if best_model_name == "LSTM":
            ...
            input_array = np.arange(len(input_array)).reshape(-1, 1)  # Flatten to 1D

            # Ensure the input is scaled correctly for Linear Regression
            input_array = scaler.transform(input_array)  # Transform to 2D shape

            prediction = best_model.predict(input_array)
        else:
            return jsonify({"error": "No suitable model selected."}), 500

        # Assuming the scaler is consistent with preprocessing
